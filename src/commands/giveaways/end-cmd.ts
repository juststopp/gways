...
class EndCmd extends Command {
    ...
    async run(ctx: Context) {
        ...
        // @ts-ignore;
        if(winners?.length > 0) ctx.channel.send({ content: `:tada: Congratulations ${winners.join(', ')}, you are the winner${winners.length > 1 ? 's' : ''} for **${giveaway[0].prize}**!` });
        // @ts-ignore;
        ...
        ctx.reply({ content: "Giveaway has been ended!", ephemeral: true })
    }
}
