...
manager.spawn().then(() => {
    ...
    manager.shards.forEach((shard) => {
        manager.broadcastEval((client, { shardId }) => { client.user.setActivity(`Shard#${shardId} with ${client.guilds.cache.size} guilds on this shard!`); }, { context: { shardId: shard.id } });
    });
}).catch(err => {
